...
export default function FlightList() {
  ...
  // Check if state and flightsData are defined before using
  const flightsData = state && state.flightsData ? state.flightsData : [];
  ...
  if (!flightsData || flightsData.length === 0) {
    return (
      <div className="no-flights">
        ...
        <p className="no-flight-msg">No Flights Available</p>
        <Link to="/" className="home-btn">
          Home
        </Link>
        <button onClick={() => history(-1)} className="back-btn">
          Back
        </button>
      </div>
    );
